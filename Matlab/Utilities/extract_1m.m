...
%
% program to get bathymetry from topo_9.1.img  (Smith and Sandwell bathymetry)
...
for i = 1:length(wlon);
	...
	% Open the data file
    %my_file_id=fopen("my_file_name", 'r', 'b'); %r: read only, b: big
    %endian
	fid = fopen([DatabasesDir '/topo_9.1.img'], 'r', 'b');
	if (fid < 0)
		errordlg(['Could not open database: ' DatabasesDir '/topo_9.1.img'],'Error');
	end
...
%	disp('Reading in bathymetry data');
  ...
  %  data(iblat-ilat+1,:)=fread(fid,[1,ielon(i)-iwlon(i)+1],'int16=>double');
    end
...
% toutes les valeurs sont negatives
if (region(3)<0 & isempty(find(vlon<0)))
   vlon = vlon-360; 
end